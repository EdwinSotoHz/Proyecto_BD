...
{
    public class GroupsAndWorkshopsPdfGenerator
    {
        private readonly List<VistaGruposTallere> _data;

        public GroupsAndWorkshopsPdfGenerator(List<VistaGruposTallere> data)
        {
            _data = data;
        }
        ...
        public byte[] GeneratePdf()
        {
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                AddTitle(document, "LISTADO DE GRUPOS Y TALLERES");
                AddTable(document);
        ...
        private void AddTable(Document document)
        {
            ...
            // Anchuras relativas ajustadas para evitar solapamiento
        ...
        1.5f  // Precio
            ...
            table.AddCell(new PdfPCell(new Phrase("Grupo ID", headerFont)) { BackgroundColor = headerBg });
            table.AddCell(new PdfPCell(new Phrase("Nombre Grupo", headerFont)) { BackgroundColor = headerBg });
            table.AddCell(new PdfPCell(new Phrase("Horario", headerFont)) { BackgroundColor = headerBg });
            table.AddCell(new PdfPCell(new Phrase("Aula", headerFont)) { BackgroundColor = headerBg });
            table.AddCell(new PdfPCell(new Phrase("Estado", headerFont)) { BackgroundColor = headerBg });
            table.AddCell(new PdfPCell(new Phrase("Taller", headerFont)) { BackgroundColor = headerBg });
            table.AddCell(new PdfPCell(new Phrase("Duración", headerFont)) { BackgroundColor = headerBg });
            table.AddCell(new PdfPCell(new Phrase("Precio", headerFont)) { BackgroundColor = headerBg });

            Font contentFont = FontFactory.GetFont(FontFactory.HELVETICA, 7, BaseColor.Black);

            foreach (var item in _data)
            {
                table.AddCell(new PdfPCell(new Phrase(item.GrupoId.ToString(), contentFont)));
                table.AddCell(new PdfPCell(new Phrase(item.NombreGrupo, contentFont)));
                table.AddCell(new PdfPCell(new Phrase(item.Horario, contentFont)));
                table.AddCell(new PdfPCell(new Phrase(item.Aula ?? "-", contentFont)));
                table.AddCell(new PdfPCell(new Phrase(item.Estado ?? "-", contentFont)));
                table.AddCell(new PdfPCell(new Phrase(item.NombreTaller, contentFont)));
                table.AddCell(new PdfPCell(new Phrase(item.Duracion ?? "-", contentFont)));
                table.AddCell(new PdfPCell(new Phrase(item.Precio?.ToString("C2") ?? "-", contentFont)));
            }
